# 👋 Sobre mí

Mi nombre es **Germán Vidal (VidAppSec)**.  
Soy técnico de infraestructura que trabaja en entornos críticos de Centro de Datos (CPD) y, en paralelo, me estoy formando activamente en ciberseguridad ofensiva con el objetivo de convertirme en **Pentester Junior** o **Analista SOC L1**.

---

## 🎓 Formación y estudios

- **Analista en Infraestructura y Ciberseguridad** – ORT Uruguay (en curso)  
- **Hacking Ético y Gestión de Incidentes** – ORT Uruguay  
- **Desarrollador Web** – UTU (título técnico obtenido)  

---

## 🧪 Experiencia profesional

Actualmente trabajo en **UTE / SONDA Uruguay**, brindando soporte técnico en un **Centro de Datos crítico**, realizando:

- Monitoreo operativo de equipamiento crítico  
- Gestión y seguimiento de incidentes  
- Coordinación con proveedores externos  
- Registro técnico de eventos relevantes (UPS, generadores, aire acondicionado industrial, sistemas de energía, etc.)  
- Documentación de maniobras, tareas y mantenimientos  

Esta experiencia me dio una base sólida en **operación de infraestructura crítica**, protocolos de trabajo, procedimientos, y manejo de incidentes reales — todo muy útil para la ciberseguridad.

---

## 🔐 Enfoque en Ciberseguridad

En paralelo, entreno en seguridad ofensiva mediante:

- **Hack The Box (HTB)**  
- Explotación y análisis de vulnerabilidades  
- Enumeración avanzada  
- Escalada de privilegios Linux y Windows  
- Pruebas de explotación Web  
- Uso de herramientas como: *nmap, gobuster, burp suite, linpeas, smbclient, wireshark, etc.*

Mi objetivo profesional es avanzar hacia roles como:

- **Pentester Junior**  
- **Analista SOC L1**  
- (A futuro) Seguridad ofensiva / Red Team

---

## 📝 ¿Qué encontrarás en este portafolio?

En esta página publico:

- Write-ups completos de máquinas **HackTheBox**  
- Laboratorios y pruebas técnicas  
- Documentación clara y orientada al aprendizaje  
- Mi evolución en ciberseguridad ofensiva  

---

## 📫 Contacto

- **LinkedIn:** https://www.linkedin.com/in/german-vidal-4536b6272/  

---

Gracias por visitar mi portafolio 💚
